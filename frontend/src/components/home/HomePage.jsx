...
function MobileBusCard({ bus }) {
  return <Link href={`/buses/${bus.id}`} className="flex min-w-0 items-center gap-3 rounded-2xl border border-[var(--border)] bg-white p-3.5 shadow-[0_5px_18px_rgba(23,51,45,0.05)]">
    <span className="grid h-11 w-11 flex-none place-items-center rounded-xl bg-[#e7f5ed] text-[var(--primary)]"><BusFront size={20} /></span>
...
function MobileRouteCard({ route }) {
  ...
  return <article className="flex w-[82vw] max-w-[310px] flex-none flex-col rounded-2xl border border-[var(--border)] bg-white p-4 shadow-[0_5px_18px_rgba(23,51,45,0.05)]">
    <div className="flex items-center justify-between gap-3"><span className="rounded-lg bg-[#e7f5ed] px-2.5 py-1.5 text-xs font-extrabold text-[var(--primary-dark)]">{routeLabel}</span>{route.isActive !== false && <span className="text-[10px] font-bold text-[var(--success)]">● Active</span>}</div>
    ...
    <Link href={`/routes/${routeId}`} className="mt-3 inline-flex min-h-10 items-center gap-1 border-t border-[var(--border)] pt-3 text-xs font-bold text-[var(--primary)]">View route <ArrowRight size={14} /></Link>
  </article>;
}
...
export default function HomePage() {
  ...
  return <div className="public-shell min-h-screen overflow-x-hidden bg-[var(--background)]">
    ...
    <main>
      <section className="relative overflow-hidden border-b border-[var(--border)] bg-[#edf7f2]">
        <div className="mx-auto grid max-w-7xl items-center gap-10 px-4 pb-6 pt-7 sm:px-6 sm:py-16 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-20">
          <div className="relative z-10">
            ...
            <p className="text-xs font-extrabold uppercase tracking-[0.14em] text-[var(--primary)] md:hidden">Plan your next journey</p>
            <h1 className="mt-2 max-w-2xl text-[2rem] font-extrabold leading-[1.08] tracking-[-0.04em] text-[var(--foreground)] sm:text-5xl lg:mt-5 lg:text-6xl"><span className="md:hidden">Where are you going?</span><span className="hidden md:inline">Smart Public Transit <span className="text-[var(--primary)]">for Faisalabad</span></span></h1>
            ...
            <div className="mt-8 hidden flex-col gap-3 sm:flex-row md:flex"><Link href="#live-transit" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[var(--primary)] px-5 text-sm font-semibold text-white transition hover:bg-[var(--primary-dark)]">View Live Buses <ArrowRight size={17} /></Link><Link href="#routes" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-[var(--border)] bg-white px-5 text-sm font-semibold text-[var(--foreground)] transition hover:border-[var(--primary)] hover:text-[var(--primary)]">Explore Routes <Map size={17} /></Link></div>
            <div className="mt-8 hidden flex-wrap items-center gap-x-5 gap-y-2 text-sm text-[var(--muted)] md:flex"><span className="inline-flex items-center gap-2"><CheckCircle2 size={16} className="text-[var(--primary)]" /> No login required</span><span className="inline-flex items-center gap-2"><CheckCircle2 size={16} className="text-[var(--primary)]" /> Live network updates</span></div>
          </div>
          <div className="relative mx-auto hidden w-full max-w-lg md:block lg:mx-0 lg:justify-self-end"><div className="rounded-[2rem] border border-[#cfe5dc] bg-white p-3 shadow-[0_24px_60px_rgba(23,51,45,0.12)]"><div className="relative h-[310px] overflow-hidden rounded-[1.5rem] bg-[#e5f1ec] sm:h-[350px]"><div className="absolute inset-0 opacity-70" style={{ backgroundImage: "linear-gradient(#c9dfd6 1px, transparent 1px), linear-gradient(90deg, #c9dfd6 1px, transparent 1px)", backgroundSize: "42px 42px" }} /><div className="absolute left-[12%] top-[60%] h-1 w-[72%] rotate-[-22deg] rounded-full bg-[var(--primary)] shadow-[0_0_0_5px_#087f5b22]" /><span className="absolute left-[18%] top-[52%] h-4 w-4 rounded-full border-4 border-white bg-[var(--primary)] shadow-md" /><span className="absolute left-[47%] top-[38%] h-4 w-4 rounded-full border-4 border-white bg-[var(--primary)] shadow-md" /><span className="absolute right-[16%] top-[26%] h-4 w-4 rounded-full border-4 border-white bg-[var(--primary)] shadow-md" /><span className="absolute left-[45%] top-[32%] grid h-11 w-11 place-items-center rounded-2xl bg-[var(--primary)] text-white shadow-lg shadow-[#087f5b55]"><BusFront size={23} /></span><div className="absolute bottom-4 left-4 right-4 flex items-center justify-between rounded-xl border border-white/70 bg-white/90 px-4 py-3 shadow-lg backdrop-blur"><div><p className="text-xs text-[var(--muted)]">Network status</p><p className="mt-1 text-sm font-bold text-[var(--foreground)]">{transitLoading ? "Checking live service…" : `${activeBuses.length} ${activeBuses.length === 1 ? "bus" : "buses"} active now`}</p></div><Badge tone={activeBuses.length ? "success" : "neutral"}>{activeBuses.length ? "Live" : "Quiet"}</Badge></div></div></div></div>
        </div>
      </section>

      <section id="transit-search" className="relative z-20 mx-auto max-w-5xl scroll-mt-24 px-4 pt-4 sm:px-6 md:-mt-7 md:pt-0"><Card className="p-3 sm:p-4"><form onSubmit={handleSearch} className="flex flex-col gap-3 sm:flex-row"><label className="flex min-h-13 flex-1 items-center gap-3 rounded-xl border border-[#b6d9ca] bg-white px-4 focus-within:border-[var(--primary)] focus-within:ring-4 focus-within:ring-[#d8f1e6]"><Search size={19} className="shrink-0 text-[var(--primary)]" /><input ref={searchRef} value={searchTerm} onChange={(event) => { setSearchTerm(event.target.value); setHasSearched(false); }} className="min-w-0 flex-1 bg-transparent text-sm text-[var(--foreground)] outline-none placeholder:text-[var(--muted)]" placeholder="Route, bus or stop" aria-label="Search routes, buses or stops" /></label><button type="submit" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[var(--primary)] px-6 text-sm font-bold text-white hover:bg-[var(--primary-dark)]"><Search size={17} /> Find transit</button></form>{hasSearched && <p className="mt-3 border-t border-[var(--border)] px-1 pt-3 text-sm text-[var(--muted)]">{matchingBuses.length + matchingRoutes.length > 0 ? `${matchingBuses.length + matchingRoutes.length} matching transit result${matchingBuses.length + matchingRoutes.length === 1 ? "" : "s"}.` : "No matching transit found. Try a route, bus number or stop."}</p>}</Card></section>
      ...
      <section id="live-transit" className="mx-auto max-w-7xl scroll-mt-20 px-4 py-8 sm:px-6 md:py-16 lg:px-8 lg:py-20"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--primary)]">Right now</p><h2 className="mt-1 text-xl font-extrabold tracking-tight text-[var(--foreground)] sm:text-3xl md:mt-2">Live Transit</h2><p className="mt-1 text-xs text-[var(--muted)] sm:text-sm md:mt-2">Buses currently operating across Faisalabad.</p></div><Link href="/live-map" className="inline-flex min-h-10 flex-none items-center gap-1 text-xs font-bold text-[var(--primary)] sm:text-sm">View all <ArrowRight size={15} /></Link></div>
        {transitLoading ? <div className="mt-4 grid gap-3 md:mt-8 md:grid-cols-2 lg:grid-cols-3">{[1, 2, 3].map((item) => <div key={item} className="h-20 animate-pulse rounded-2xl border border-[var(--border)] bg-white md:h-56" />)}</div> : transitError ? <div className="mt-4"><EmptyState title="Live transit is unavailable" description="We couldn't retrieve current bus information right now." /></div> : displayedBuses.length ? <><div className="mt-4 grid gap-2.5 md:hidden">{displayedBuses.slice(0, 3).map((bus) => <MobileBusCard key={bus.id} bus={bus} />)}</div><div className="mt-8 hidden gap-4 md:grid md:grid-cols-2 lg:grid-cols-3">{displayedBuses.slice(0, 6).map((bus) => <BusCard key={bus.id} bus={bus} />)}</div></> : <div className="mt-4"><EmptyState title={hasSearched ? "No active buses found" : "No buses operating right now"} description={hasSearched ? "Try another bus number, route, or stop." : "Live details will appear when service is active."} /></div>}
      </section>

      <section className="border-y border-[var(--border)] bg-white"><div className="mx-auto grid max-w-7xl gap-4 px-4 py-8 sm:px-6 md:gap-8 md:py-16 lg:grid-cols-[1fr_1.55fr] lg:items-center lg:px-8 lg:py-20"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--primary)]">Live network map</p><h2 className="mt-1 text-xl font-extrabold tracking-tight text-[var(--foreground)] sm:text-3xl md:mt-2">Your journey, in view.</h2><p className="mt-2 max-w-md text-sm leading-6 text-[var(--muted)]">See active buses, stops and route movement together.</p><div className="mt-4 hidden max-w-sm rounded-2xl border border-[var(--border)] bg-[#f8fbfa] p-4 md:block"><p className="text-sm font-bold text-[var(--foreground)]">{transitLoading ? "Checking service…" : `${activeBuses.length} active ${activeBuses.length === 1 ? "bus" : "buses"}`}</p><p className="mt-1 text-xs text-[var(--muted)]">Map data comes from the current transit network.</p></div></div><div><div className="public-home-map relative overflow-hidden rounded-2xl border border-[var(--border)] bg-[#eaf3ef] p-1 shadow-[0_14px_36px_rgba(23,51,45,0.08)]"><ClientTransitMap center={[31.416, 73.078]} zoom={13} className="h-[270px] rounded-xl sm:h-[340px] lg:h-[450px]">{routeGeometries.map(({ route, stops: routeStops }) => { const positions = routeStops.filter((stop) => stop.latitude != null && stop.longitude != null).map((stop) => [stop.latitude, stop.longitude]); return positions.length > 1 ? <RoutePolyline key={getRouteId(route)} positions={positions} /> : null; })}<MapControls />{activeBuses.filter((bus) => bus.position).map((bus) => <BusMarker key={bus.id} position={bus.position} bus={bus} />)}{stops.map((stop) => <StopMarker key={stop.id} position={stop.position} stop={stop} />)}</ClientTransitMap></div><Link href="/live-map" className="mt-3 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-[#b6d9ca] bg-white text-sm font-bold text-[var(--primary-dark)] md:hidden"><MapPin size={17} /> View live map</Link></div></div></section>

      <section id="routes" className="mx-auto max-w-7xl scroll-mt-20 px-4 py-8 sm:px-6 md:py-16 lg:px-8 lg:py-20"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--primary)]">Plan ahead</p><h2 className="mt-1 text-xl font-extrabold tracking-tight text-[var(--foreground)] sm:text-3xl md:mt-2">Popular Routes</h2><p className="mt-1 text-xs text-[var(--muted)] sm:text-sm md:mt-2">Find a route for your next journey.</p></div><Link href="/routes" className="inline-flex min-h-10 flex-none items-center gap-1 text-xs font-bold text-[var(--primary)] sm:text-sm">View all <ArrowRight size={15} /></Link></div>
        {transitLoading ? <div className="mt-4 flex gap-3 overflow-hidden md:grid md:grid-cols-3">{[1, 2, 3].map((item) => <div key={item} className="h-44 w-[82vw] max-w-[310px] flex-none animate-pulse rounded-2xl border border-[var(--border)] bg-white md:w-auto" />)}</div> : displayedRoutes.length ? <><div className="public-route-scroll -mx-4 mt-4 flex gap-3 overflow-x-auto px-4 pb-2 md:hidden">{displayedRoutes.slice(0, 5).map((route) => <MobileRouteCard key={getRouteId(route)} route={route} />)}</div><div className="mt-8 hidden gap-4 md:grid md:grid-cols-2 lg:grid-cols-3">{displayedRoutes.slice(0, 6).map((route) => <RouteCard key={getRouteId(route)} route={route} />)}</div></> : <div className="mt-4"><EmptyState title="No routes found" description="Try another route name, starting point, or destination." /></div>}
      </section>

      <section className="border-y border-[var(--border)] bg-[#f0f7f4]"><div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 md:py-16 lg:px-8 lg:py-20"><div className="max-w-xl"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--primary)]">Why Smart Safar</p><h2 className="mt-1 text-xl font-extrabold tracking-tight text-[var(--foreground)] sm:text-3xl md:mt-2">Better information for every trip.</h2></div><div className="mt-4 grid grid-cols-2 gap-2.5 md:mt-8 md:grid-cols-4 md:gap-4">{benefits.map(({ title, text, icon: Icon }) => <Card key={title} className="p-4 md:p-5"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#e0f2e9] text-[var(--primary)] md:h-10 md:w-10"><Icon size={18} /></span><h3 className="mt-3 text-sm font-bold text-[var(--foreground)] md:mt-5 md:text-base">{title}</h3><p className="mt-1 text-xs leading-5 text-[var(--muted)] md:mt-2 md:text-sm md:leading-6">{text}</p></Card>)}</div></div></section>

      <section className="mx-auto hidden max-w-7xl px-4 py-16 sm:px-6 md:block lg:px-8 lg:py-20"><Card className="grid gap-8 overflow-hidden bg-[#173f36] p-8 text-white lg:grid-cols-[1fr_auto] lg:items-center lg:p-10"><div><Badge className="bg-white/15 text-[#c8f0dd]">Built for safer journeys</Badge><h2 className="mt-4 text-3xl font-bold tracking-tight">Safer &amp; More Reliable Travel</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-[#cde1d9]">Travel with better context. Women Safety helps passengers share the live location of their selected bus with trusted contacts while keeping their personal location private. Bus Trust Score reflects punctuality and historical arrival performance, while route change alerts keep you informed.</p></div><span className="grid h-16 w-16 place-items-center rounded-2xl bg-[#2d705e] text-[#d5f5e6] lg:mr-5"><ShieldCheck size={32} /></span></Card></section>

      <section className="border-t border-[var(--border)] bg-white"><div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-4 px-4 py-8 sm:px-6 md:flex-row md:items-center md:py-14 lg:px-8"><div><h2 className="text-xl font-extrabold tracking-tight text-[var(--foreground)] sm:text-2xl">Ready to plan your journey?</h2><p className="mt-1 text-sm text-[var(--muted)] md:mt-2">Find your route and track your bus with live information.</p></div><div className="grid w-full grid-cols-2 gap-2.5 sm:w-auto"><Link href="/live-map" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[var(--primary)] px-4 text-sm font-semibold text-white">Track a Bus</Link><Link href="/routes" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[var(--border)] bg-white px-4 text-sm font-semibold text-[var(--foreground)]">View Routes</Link></div></div></section>
    </main>
    <Footer />
  </div>;
}
