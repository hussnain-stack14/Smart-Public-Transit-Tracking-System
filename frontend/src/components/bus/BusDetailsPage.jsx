...
export default function BusDetailsPage({ busId }) {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[var(--background)]"><Navbar /><main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10"><div><Link href="/live-map" aria-label="Back to live buses" className="inline-grid h-11 w-11 place-items-center rounded-xl border border-[var(--border)] bg-white text-[var(--primary)] hover:border-[var(--primary)]"><ArrowLeft size={19} /></Link></div><header className="mt-6 flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl bg-[#e3f3ec] text-[var(--primary)]"><BusFront size={20} /></span><Badge tone={operating ? "success" : "neutral"}>{operating ? "Currently operating" : "Not operating"}</Badge></div><h1 className="mt-4 text-3xl font-bold tracking-tight text-[var(--foreground)] sm:text-4xl">{liveBus.busNumber}</h1><p className="mt-2 flex items-center gap-2 text-base text-[var(--muted)]"><span>{routeLabel}</span><span>Â·</span><span>{routeName}</span></p></div><Link href={routeId ? `/routes/${routeId}` : "/routes"} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[var(--border)] bg-white px-5 text-sm font-semibold text-[var(--foreground)] transition hover:border-[var(--primary)] hover:text-[var(--primary)]"><RouteIcon size={16} /> View full route</Link></header>
