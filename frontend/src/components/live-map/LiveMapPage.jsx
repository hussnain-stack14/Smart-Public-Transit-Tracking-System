...
export default function LiveMapPage() {
  ...
  const mapPositions = useMemo(() => buses.filter((bus) => bus.status === "active").map((bus) => bus.position).filter(Boolean), [buses]);
  ...
  function locateUser(map) {
    ...
    navigator.geolocation.getCurrentPosition(({ coords }) => {
      ...
      setUserLocation({ position, accuracy: coords.accuracy });
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[var(--background)]"><Navbar /><main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10"><header className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--primary-ink)]">Live transit</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-[var(--foreground)] sm:text-4xl">Track buses in real time.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--muted)]">See active buses, routes, stops and estimated arrival times across Faisalabad.</p></div><Badge tone={connection === "live" ? "success" : connection === "offline" ? "danger" : "warning"}><span className="mr-1.5">{connection === "live" ? "●" : connection === "offline" ? "×" : "○"}</span>{connection === "live" ? "Live network" : connection === "offline" ? "Offline" : "Connecting"}</Badge></header>
      ...
      {connection === "connecting" && <div className="mt-6 flex items-center gap-2 rounded-xl border border-[var(--border)] bg-white px-4 py-3 text-sm text-[var(--muted)]"><Wifi size={16} className="shrink-0 text-[var(--primary-ink)]" /> Connecting to live transit updates...</div>}
      ...
      <section className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1.7fr)_minmax(300px,0.8fr)] lg:items-start"><div className="relative overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface-subtle)] p-1 shadow-[0_14px_36px_rgba(15,23,42,0.08)]"><div className="relative"><ClientTransitMap center={[31.416, 73.078]} zoom={13} className="h-[min(66vh,600px)] min-h-[420px] rounded-xl"><MapViewport positions={mapPositions.length ? mapPositions : [routeLine[0], routeLine[routeLine.length - 1]]} focusKey={`${selectedRoute}-${filteredBuses.length}`} /><RoutePolyline positions={selectedRouteLine} color={selectedRoute === "All routes" ? "var(--primary)" : "var(--primary-dark)"} />{operatingBuses.map((bus) => bus.position && <BusMarker key={bus.id} position={bus.position} bus={bus} onSelect={selectBus} />)}{stops.map((stop) => <StopMarker key={stop.id} position={stop.position} stop={stop} onSelect={(nextStop) => { setSelectedStop(nextStop); setSelectedBus(null); }} />)}<UserLocationMarker location={userLocation} /><MapControls onLocate={locateUser} onShowAll={showAllBuses} /></ClientTransitMap><LiveMapLegend showUser={Boolean(userLocation)} /></div>{selectedBus && <div className="absolute bottom-4 left-4 right-4 z-[500] sm:max-w-sm"><BusDetailsPanel bus={selectedBus} onClose={() => setSelectedBus(null)} /></div>}{selectedStop && <div className="absolute bottom-4 left-4 right-4 z-[500] sm:max-w-sm"><StopDetailsPanel stop={selectedStop} onClose={() => setSelectedStop(null)} /></div>}</div>

        <aside className="grid gap-4"><Card className="p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-[var(--primary-ink)]">Network now</p><h2 className="mt-1 text-xl font-bold text-[var(--foreground)]">Fleet</h2></div><span className="inline-flex items-center gap-1.5 text-xs font-semibold text-[var(--success)]"><Wifi size={14} /> Updates on</span></div><p className="mt-2 text-sm text-[var(--muted)]">{filteredBuses.length} of {buses.length} buses shown</p></Card>{dataLoading && !buses.length ? <LoadingSpinner label="Loading live buses..." /> : dataError && !buses.length ? <EmptyState title="Transit data unavailable" description="Unable to load buses. Check your connection and reload the page." /> : filteredBuses.length ? filteredBuses.map((bus) => <div key={bus.id} className="cursor-pointer" onClick={() => selectBus(bus)}><BusCard bus={bus} /></div>) : <EmptyState title="No active buses" description="There are currently no buses matching your filters." action={<Button type="button" variant="secondary" onClick={() => { setSearchTerm(""); setSelectedRoute("All routes"); setSelectedStatus("All"); }}>Clear filters</Button>} />}</aside></section>

      <section className="mt-6 grid gap-4 md:grid-cols-3"><Card className="p-4"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[var(--primary-soft)] text-[var(--primary-ink)]"><MapPinned size={17} /></span><div><p className="text-sm font-bold text-[var(--foreground)]">{stops.length} stops visible</p><p className="text-xs text-[var(--muted)]">Tap a stop for route information.</p></div></div></Card>{popularRoutes.slice(0, 2).map((route) => <Card key={route.id} className="p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-[var(--primary-ink)]">{route.number}</p><p className="mt-1 text-sm font-bold text-[var(--foreground)]">{route.name}</p></div><Link href={`/routes/${route.id}`} className="text-xs font-semibold text-[var(--primary-ink)] hover:text-[var(--primary-ink)]">View route</Link></div><p className="mt-2 text-xs text-[var(--muted)]">{route.stops} stops · {route.buses}</p></Card>)}</section></main><Footer /></div>;
}
