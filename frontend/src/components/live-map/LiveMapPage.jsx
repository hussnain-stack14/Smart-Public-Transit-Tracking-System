...
export default function LiveMapPage() {
  ...
  const mapPositions = filteredBuses.map((bus) => bus.position).filter(Boolean);
  const routeOptions = ["All routes", ...routes.map((route) => getRouteId(route))];
  const visibleStops = (selectedGeometry?.stops || routeStops.flatMap(({ stops: routeStopList }) => routeStopList)).filter((stop) => stop.latitude != null && stop.longitude != null).map((stop) => ({ ...stop, id: stop._id, name: stop.stopName, position: [stop.latitude, stop.longitude] }));
  const routeCards = routes.slice(0, 2).map((route) => ({ ...route, number: getRouteId(route), name: route.routeName, start: route.startPoint, end: route.endPoint, stops: routeStops.find((item) => getRouteId(item.route) === getRouteId(route))?.stops.length || 0, buses: `${apiBuses.filter((bus) => bus.routeId === getRouteId(route) && bus.status === "active").length} active` }));
  ...
  function showAllBuses(map) {
    const positions = buses.map((bus) => bus.position).filter(Boolean);
