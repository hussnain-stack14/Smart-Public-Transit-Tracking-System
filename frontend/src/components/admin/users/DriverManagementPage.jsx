...
function DriverOperations() {
  ...
  const assignedBus = (driver) => driver.assignedBus ? <Link href={"/buses/" + driver.assignedBus} className="break-all font-semibold text-[var(--primary-ink)]">{buses.find((bus) => bus._id === driver.assignedBus)?.busNumber || driver.assignedBus}</Link> : <span className="text-[var(--muted)]">Unassigned</span>;
  ...
  return <>
    ...
    <header className="mt-3 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--primary-ink)]">Fleet operations</p><h1 className="mt-2 text-3xl font-bold sm:text-4xl">Driver Management</h1><p className="mt-2 text-sm text-[var(--muted)]">Manage driver accounts and their bus assignments.</p></div><div className="flex flex-wrap gap-3"><Button type="button" variant="secondary" className="min-h-12 gap-2" onClick={refresh} disabled={loading || busLoading}><RefreshCw size={16} aria-hidden="true" />{loading || busLoading ? "Refreshing..." : "Refresh drivers"}</Button><Button type="button" className="min-h-12" onClick={() => setEditor({ driver: null })}>Add Driver</Button></div></header>
