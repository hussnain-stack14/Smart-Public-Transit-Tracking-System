...
export function BusTable({ buses, loading, onEdit, onDelete, onAssign, onUnassign, onAddClick, editDisabled }) {
  ...
  return <>
    <div className="hidden overflow-hidden rounded-2xl border border-[var(--border)] bg-white xl:block"><table className="w-full table-fixed text-left text-sm"><thead className="bg-[#f0f7f4] text-xs text-[var(--muted)]"><tr><th scope="col" className="p-4">Bus</th><th scope="col" className="p-4">Route</th><th scope="col" className="p-4">Assigned driver</th><th scope="col" className="w-28 p-4">Status</th><th scope="col" className="w-28 p-4">Available seats</th><th scope="col" className="w-64 p-4">Actions</th></tr></thead><tbody>{buses.map((bus) => <tr key={bus._id} className="border-t border-[var(--border)] align-top"><td className="p-4"><Link href={"/buses/" + bus._id} className="break-words font-bold text-[var(--primary)]">{bus.busNumber}</Link><p className="mt-2 break-all text-xs text-[var(--muted)]">{bus._id}</p></td><td className="break-words p-4">{bus.route?._id ? <Link href={"/routes/" + bus.route._id} className="font-semibold text-[var(--primary)]">{bus.route.routeName}</Link> : "Unavailable"}</td><td className="p-4">{driver(bus)}</td><td className="p-4"><Badge tone={statusTones[bus.status] || "neutral"}>{bus.status || "Unavailable"}</Badge></td><td className="p-4">{bus.availableSeats ?? "Unavailable"} / {bus.capacity ?? "Unavailable"}</td><td className="p-4">{actions(bus)}</td></tr>)}</tbody></table></div>
    <div className="grid gap-4 md:grid-cols-2 xl:hidden">{buses.map((bus) => <Card key={bus._id} className="min-w-0 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><Link href={"/buses/" + bus._id} className="break-words text-xl font-bold text-[var(--primary)]">{bus.busNumber}</Link><Badge tone={statusTones[bus.status] || "neutral"}>{bus.status || "Unavailable"}</Badge></div><p className="mt-2 break-all text-xs text-[var(--muted)]">Bus ID: {bus._id}</p><dl className="mt-4 grid gap-4 text-sm"><div><dt className="text-[var(--muted)]">Route</dt><dd className="mt-1 break-words font-semibold">{bus.route?.routeName || "Unavailable"}</dd></div><div><dt className="text-[var(--muted)]">Assigned driver</dt><dd className="mt-1">{driver(bus)}</dd></div><div><dt className="text-[var(--muted)]">Available seats / Capacity</dt><dd className="mt-1 font-semibold">{bus.availableSeats ?? "Unavailable"} / {bus.capacity ?? "Unavailable"}</dd></div></dl><div className="mt-5 border-t border-[var(--border)] pt-4">{actions(bus)}</div></Card>)}</div>
  </>;
}
