...
export function Navbar() {
  ...
  return <header className="sticky top-0 z-50 border-b border-[var(--border)] bg-[#f5f8f7]/95 backdrop-blur">
    <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
      ...
      {isAuthenticated ? <button type="button" onClick={handleLogout} className="hidden items-center gap-2 rounded-xl bg-[var(--primary)] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[var(--primary-dark)] md:inline-flex">Log out</button> : <div className="hidden items-center gap-2 md:flex"><Link href="/register" className="rounded-xl px-3 py-2.5 text-sm font-semibold text-[var(--primary)] transition hover:bg-white">Create account</Link><Link href="/login" className="inline-flex items-center gap-2 rounded-xl bg-[var(--primary)] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[var(--primary-dark)]">Sign In <ArrowRight size={15} /></Link></div>}
    ...
    <div className={cn("border-t border-[var(--border)] bg-white px-4 py-3 md:hidden", !isOpen && "hidden")}>
      <nav className="grid gap-1" aria-label="Mobile navigation">{links.map((link) => <Link key={link.href} href={link.href} onClick={() => setIsOpen(false)} className={cn("rounded-xl px-3 py-3 text-sm font-semibold hover:bg-[#eef7f3]", pathname === link.href ? "bg-[#eef7f3] text-[var(--primary)]" : "text-[var(--foreground)]")}>{link.label}</Link>)}{isAuthenticated ? <button type="button" onClick={handleLogout} className="mt-2 inline-flex items-center justify-center rounded-xl bg-[var(--primary)] px-4 py-3 text-sm font-semibold text-white">Log out</button> : <><Link href="/login" onClick={() => setIsOpen(false)} className="mt-2 inline-flex items-center justify-center gap-2 rounded-xl bg-[var(--primary)] px-4 py-3 text-sm font-semibold text-white">Sign In <ArrowRight size={15} /></Link><Link href="/register" onClick={() => setIsOpen(false)} className="inline-flex items-center justify-center rounded-xl border border-[var(--border)] px-4 py-3 text-sm font-semibold text-[var(--primary)]">Create account</Link></>}</nav>
    </div>
  </header>;
}
